function [LOAD,TR] = Pitch_up_analysis(COM,TR,loads,LOAD,PlotFlag)
...
alpha = loads.alpha; % alpha definition wrt axis of the flow, can be used only
%                      for the pitch up, if alpha were to go to 0, max acc
%                      on z: no sense!!!!-> ok only for pitch up
% to eliminate alpha contribution put alpha =pi/2;

delta = deg2rad(12); % pitch up maneuver action
T = loads.T;
...
x_cp_tot0 = 15;
x_cp_nose = (2/3) * TR.Fair.fair_length; 

%% Load analysis:

% Verify that global balance is correct: 

P_balance= T*cos(delta) - D - m_tot0*(nx*g0 + g0*sin(alpha));
R_balance = m_tot0*(nz*g0 + g0*cos(alpha)) - T*sin(delta) - L - L_nose -L_fin;
M_balance = -L*(l_tot - x_cp_tot0) + m_tot0*(g0*nz + g0*cos(alpha))*(l_tot - x_com_tot0) - L_nose*(l_tot-x_cp_nose);

if P_balance> 1

nx =  ((T*cos(delta) - D)/(m_tot0*g0)) - sin(alpha);

end

if R_balance> 1

nz =   (T*sin(delta) + L + L_nose +L_fin)/(m_tot0*g0) - cos(alpha);

end

if M_balance> 1

x_cp_tot0 =  l_tot - ((m_tot0*(g0*nz + g0*cos(alpha))*(l_tot - x_com_tot0) - L_nose*(l_tot-x_cp_nose))/L);
...
P_1 = D1 + m1*g0*(nx + sin(alpha));

R_1 = L1 - m1*g0*(nz + cos(alpha));
...
P_2 = P_1 + D2 + m2*g0*(nx + sin(alpha));

R_2 = L2 - m2*g0*(nz + cos(alpha)) + R_1;
...
P_3 = P_2 + D3 + m3*g0*(nx+ sin(alpha));

R_3 = L3 - m3*g0*(nz +cos(alpha)) + R_2;
...
P_4 = P_3 + D4 + m4*g0*(nx+ sin(alpha)) - T*cos(delta);

R_4 = L4 - m4*g0*(nz +cos(alpha)) + R_3 + T*sin(delta);
