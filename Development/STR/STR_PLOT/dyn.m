function [dY, parout] = dyn(t,y, stage, params, current_stage, delta)
   ...
   % [N]       - Lift force acting on the rocket
    ...
    if nargout > 1
        ...
        parout.F_x = F_x;
        parout.F_z = F_z;
        parout.T_x =+T*cos(delta)*cos(theta);
        parout.T_z =+T*cos(delta)*sin(theta);
        parout.m = m;
        parout.D = -D*sin(pi/2-gamma) -L*cos(pi/2-gamma);
        parout.L = -D*cos(pi/2-gamma) + L*sin(pi/2-gamma);
        parout.W = -g*m;
        parout.dcm = dcm;
        parout.theta = theta;
        parout.rot_angle = rot_angle;
        parout.gamma = gamma;

%        Momentum balance forces:

        parout.T = T;
        parout.D_M = D;
        parout.L_M = L;
        parout.A_M = D*sin(alpha)-L*cos(alpha);
        parout.Aer_arm = (stage.xcp - stage.xcg);
        parout.T_arm = (stage.length - stage.xcg);

%        Body frame balance forces:

        parout.F_x_body = Fxz_body(1,1);
        parout.F_z_body = Fxz_body(2,1);
        parout.D_body = parout.D*dcm(2,2) + parout.L*dcm(2,1);
        parout.L_body = parout.D*dcm(1,2) + parout.L*dcm(1,1);
        parout.Wx_body = parout.W*dcm(2,1);
        parout.Wz_body = parout.W*dcm(1,1);
        parout.Tx_body = parout.T_x*dcm(2,2) + parout.T_z*dcm(2,1) ;
        parout.Tz_body = parout.T_x*dcm(1,2) + parout.T_z*dcm(1,1) ;
